...
import time
import config
from DataReader import FeatureDictionary, sparse_representation
...
class DeepFM(object):
    ...
    def __init__(self, config):
        ...
        self.all_field_cnt = all_field_cnt

    def add_placeholders(self):
        self.X = tf.placeholder('float32', [None, self.p])
        self.y = tf.placeholder('int64', [None,])
        # index of none-zero features
        #self.field_inds = tf.placeholder('int64', [None, self.max_cols])
        self.sp_inds = tf.placeholder('int64',[None, 2])
        self.sp_value = tf.placeholder('int64', [None,])
    ...
    def inference(self):
        ...
        with tf.variable_scope('DNN',reuse=False):
            ...
            maxcol_zero = tf.get_variable('zeor_line',shape=[1,self.k],initializer=tf.zeros_initializer())
            v_concat = tf.concat([v,maxcol_zero],0)
            sp_tensor = tf.SparseTensor(indices = self.sp_inds, values = self.sp_value, dense_shape=[self.all_field_cnt* self.X.shape[1].value,self.max_cols])
            embedding = tf.nn.embedding_lookup_sparse(v_concat, sp_tensor ,None, combiner='mean')
            y_embedding_input = tf.reshape(embedding, [-1, self.all_field_cnt*self.k])
            # first hidden layer
            w1 = tf.get_variable('w1_dnn', shape=[self.all_field_cnt*self.k, 200],
                                 initializer=tf.truncated_normal_initializer(mean=0,stddev=1e-2))
...
def train_model(sess, model, epochs=10, print_every=500):
    ...
    for e in range(epochs):
        ...
        for data in train_data:
            ...
            batch_idx = []
            batch_sp_value = []
            # add = [(lambda x,l : i+l) for i in x]
            for i in range(actual_batch_size):
                sample = data.iloc[i,:]
                array,idx,sp_value = sparse_representation(sample,feat_dict, feature_length, max_cols)
                for j in idx:
                    j[0] += all_field_cnt*i
                batch_X.append(array[:-1])
                batch_y.append(array[-1])
                batch_idx.extend(idx)
                batch_sp_value.extend(sp_value)

            batch_X = np.array(batch_X)
            batch_y = np.array(batch_y)
            batch_idx = np.array(batch_idx)
            batch_sp_value = np.array(batch_sp_value)
            #print(batch_X.shape, batch_y.shape)
            #print(batch_idx.shape, batch_sp_value.shape)
            #field_array = np.array(field_array)
            # create a feed dictionary for this batch
            feed_dict = {model.X: batch_X, model.y: batch_y,
                         model.sp_inds: batch_idx,model.sp_value: batch_sp_value, model.keep_prob:1}
...
def validation_model(sess, model, print_every=50):
    ...
    validation_data = pd.read_csv('./data/train_feature.csv',
                                  chunksize=model.batch_size)
    # testing step
    valid_step = 1
    # batch_size data
...
def test_model(sess, model, print_every = 50):
    ...
    for data in test_data:
        ...
        # write to csv files
        data['label'] = y_out_prob[0][:,-1]
        if test_step == 1:
            data[['aid','uid','label']].to_csv('Deep_FM_FTRL_v1.csv', mode='a', index=False, header=True)
        else:
...
if __name__ == '__main__':
    ...
    df = FeatureDictionary(dfTrain = train, dfTest = test, cv_cols = config.CV_COLS, oh_cols = config.OH_COLS)
    ...
    with tf.Session() as sess:
        ...
        validation_model(sess, model, print_every=500)
        print('start testing...')
